        ...
        let messages: [[String: Any]] = [
            ...
            ["role": "user", "content": [
                ...
                [
                    ...
                    Assume default expiration based on possible categories: Fruits, Vegetable, Grains, Beverage, Meat, Dairy, Seafood or Other. units can be: pieces, kg, lbs , liters, ml, oz, pt.  Ignore additional information (if brands are necessary keep it), use only integers or double for unit. Combine multiple products by units if same are met. If you see date on check - calculate that it might be actually expired with days.
                    """
                ]
            ]]
        ]
...
struct AddItemView: View {
    ...
    let units = ["pieces", "kg", "lbs", "liters", "ml", "oz"]
    let categories = ["Other", "Dairy", "Meat", "Vegetable", "Fruits", "Grains", "Beverage", "Seafood"]
    ...
    var body: some View {
        ...
        .alert("No food items detected", isPresented: $showNoItemsAlert) {
            Button("OK", role: .cancel) {}
        } message: {
            Text("No food items were detected. Please try again with a clearer photo or different receipt.")
        }
    }
                ...
                if let jsonData = extractAndFixJSON(from: content) {
                    do {
                        ...
                        if parsedItems.isEmpty {
                            DispatchQueue.main.async { showNoItemsAlert = true }
                            return
                        }
                        saveItemsToFirestore(parsedItems)
                        print("✅ Parsed \(parsedItems.count) item(s) successfully")
